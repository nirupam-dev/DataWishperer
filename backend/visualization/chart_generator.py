...
class ChartGenerator:
    ...
    def _render_bar(
        ...
        colors = self._theme.get_colormap(len(data))
            ...
            if spec.show_values:
                for bar, val in zip(bars, data.values):
                    ax.text(bar.get_width() * 1.01, bar.get_y() + bar.get_height() / 2,
                            ...
                            color=self._theme.theme.text_color)
        else:
            bars = ax.bar(range(len(data)), data.values, color=colors, width=0.7,
                          edgecolor="white", linewidth=0.5)
            ax.set_xticks(range(len(data)))
            ax.set_xticklabels([str(l)[:15] for l in data.index],
                               rotation=45, ha="right", fontsize=10)
            ax.set_ylabel(spec.y_label or y_col or "Value")
            if spec.show_values and len(data) <= 15:
                for bar, val in zip(bars, data.values):
    ...
    def _render_pie(
        ...
        if spec.extra_params.get("explode_top") and len(explode) > 0:
            explode[0] = 0.08
    ...
    def _render_histogram(
        ...
        ax.axvline(mean_val, color="#FF7675", linestyle="--", linewidth=1.5, label=f"Mean: {mean_val:,.2f}")
        ax.axvline(median_val, color="#55EFC4", linestyle="--", linewidth=1.5, label=f"Median: {median_val:,.2f}")
        ax.legend(fontsize=10, facecolor=self._theme.theme.plot_bg_color, edgecolor="none",
    ...
    def _render_line(
        ...
        ax.plot(x_data, y_data, color=PALETTE_PRIMARY[0], linewidth=2,
                marker=marker, markersize=4, markerfacecolor=PALETTE_PRIMARY[1])
        ax.fill_between(range(len(y_data)) if not hasattr(x_data, "dt") else x_data,
                        y_data, alpha=0.1, color=PALETTE_PRIMARY[0])
