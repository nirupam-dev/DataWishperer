...
FEW_SHOT_EXAMPLES: list[dict[str, str]] = [
    {
        ...
        "code": (
            ...
            "result = df.groupby('category')['revenue'].mean().sort_values(ascending=False).round(2)"
        ),
    },
    {
        ...
        "code": (
            ...
            "\n"
            "# Convert date column and aggregate monthly sales\n"
            ...
            "ax.set_title('Total Sales by Month', fontsize=16, fontweight='bold', pad=15)\n"
            ...
            "plt.close()\n"
            "\n"
            "result = f'Monthly sales chart generated. Total: ${monthly.sum():,.2f}'"
        ),
    },
    {
        "question": "How many missing values are there in each column?",
        "code": (
            "# Count missing values per column and filter non-zero\n"
            "nulls = df.isnull().sum()\n"
            ...
            "    result = pd.DataFrame({'Missing Count': nulls, 'Percentage': pct})"
        ),
    },
    {
